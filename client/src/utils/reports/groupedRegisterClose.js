...
function generateReport(data, configParams) {
  ...
  let fileName = `condensado-de-caja-${fileNameDate}.pdf`;
  ...
  let subTitle = `CONDENSADO DE CAJA`;
  ...
  data.map((item, index) => {
    ...
    doc.text(`${currencyFormat(item.pay, false)}`, left + colsWidth[1], top);
...
function renderTableHeader(doc, pos, top) {
  ...
  createSubTitle(doc, "Total\nCobrado", pos + colsWidth[1], top - 2, "center");
  createSubTitle(doc, "Sucursal", pos + colsWidth[2], top);
