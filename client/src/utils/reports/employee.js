...
let colsWidth = [50, 85, 140, 205, 225, 256];
...
function generateReport(data, configParams) {
  ...
  let fileName = `reporte-empleados-${fileNameDate}.pdf`;
  ...
  let subTitle = `REPORTE DE EMPLEADOS`;
  ...
  data.map((item, index) => {
    ...
    // });
    doc.text(
      `${new Date(item.date_of_hire).toLocaleDateString("es-DO")}`,
      left + colsWidth[4] + 13,
      top,
      {
        align: "right",
