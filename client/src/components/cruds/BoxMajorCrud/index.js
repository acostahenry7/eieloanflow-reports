...
import { generateReport } from "../../../utils/reports/boxMajor";
import { currencyFormat } from "../../../utils/reports/report-helpers";
import { getBoxMajorByEmployee } from "../../../api/accounting";

function BoxMajorCrud() {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const employees = await getBoxMajorByEmployee(searchParams);
  ...
  const [columns, setColumns] = React.useState([
    ...
    {
      name: "Transacciones",
      selector: (row) => currencyFormat(row.transactions),
      sortable: true,
      reorder: true,
      omit: false,
    },
    {
      name: "Débito",
      selector: (row) => currencyFormat(row.debit),
      sortable: true,
      reorder: true,
      omit: false,
    },
    {
      name: "Crédito",
      selector: (row) => currencyFormat(row.credit),
      sortable: true,
      reorder: true,
      omit: false,
    },
