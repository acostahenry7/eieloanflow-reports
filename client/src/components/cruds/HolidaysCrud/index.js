...
function HolidaysCrud({ type }) {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const employees = await getHolidaysApi({ type, ...searchParams });
  ...
  const [columns, setColumns] = React.useState([
    ...
    },
    {
      ...
      selector: (row) => row.payroll_description,
      sortable: true,
      reorder: true,
      omit: false,
    },
    ...
    {
      name: "Monto a pagar",
      selector: (row) => currencyFormat(Math.round(row.amount_entry)),
      sortable: true,
      reorder: true,
      omit: false,
    },
